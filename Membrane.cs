...
namespace ReinforcedConcreteMembrane
{
    public abstract class Membrane
    {
        ...
        {
            // Initiate concrete
            var concrete = new Concrete.MCFT(19.6, 6);
            ...
            // Get reinforcement
            var reinforcement = new Reinforcement.Panel((6.35, 4.47), (50.55, 50.38), (steelX, steelY), 70);

            // Initiate the membrane
            var membrane = new Membrane.MCFT(concrete, reinforcement);

            // Initiate stresses
            var sig0 = 0.05 * Vector<double>.Build.DenseOfArray(new double[]
            {
               0, 0, 1
            });
            ...
            // Initiate matrices
            var epsMatrix = Matrix<double>.Build.Dense(100, 3);
            var e1Matrix = Matrix<double>.Build.Dense(100, 2);
            var sigMatrix = Matrix<double>.Build.Dense(100, 3);
            var sig1Matrix = Matrix<double>.Build.Dense(100, 2);
            var sigCMatrix = Matrix<double>.Build.Dense(100, 3);
            var sigSMatrix = Matrix<double>.Build.Dense(100, 3);
            var thetaMatrix = Matrix<double>.Build.Dense(100, 1);
            ...
            for (int ls = 1; ls <= 100; ls++)
            {
                ...
                {
                    // Do analysis by MCFT
                    membrane.Analysis2(ei, ls);

                    // Calculate residual
                    var sigr = sig - membrane.Stresses;

                    if (EquilibriumConvergence(sigr, ei - e0, it))
                    {
                        Console.WriteLine("LS = {0}, Iterations = {1}", ls, it);

                        // Set results to membrane element
                        membrane.Results();
                        ...
                        sigCMatrix.SetRow(ls - 1, membrane.ConcreteStresses);
                        sigSMatrix.SetRow(ls - 1, membrane.ReinforcementStresses);
                        e1Matrix.SetRow(ls - 1, new[] { ec1, ec2 });
                        sig1Matrix.SetRow(ls - 1, new[] { fc1, fc2 });
                        thetaMatrix[ls - 1, 0] = membrane.PrincipalAngles.theta2;
            ...
            // Save results
            DelimitedWriter.Write("D:/eps.csv", epsMatrix, ";");
            DelimitedWriter.Write("D:/e1.csv", e1Matrix, ";");
            DelimitedWriter.Write("D:/sig.csv", sigMatrix, ";");
            DelimitedWriter.Write("D:/sig1.csv", sig1Matrix, ";");
            DelimitedWriter.Write("D:/sigC.csv", sigCMatrix, ";");
            DelimitedWriter.Write("D:/sigS.csv", sigSMatrix, ";");
            DelimitedWriter.Write("D:/theta.csv", thetaMatrix, ";");
